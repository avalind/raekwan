...
def strand_absolute_count(raw, alt_f=2, alt_r=2):
    """ 
        Filter out all variants not supported by alt_f reads in forward direction
...
def filter_callstat(filename):
    dataset = pd.read_table(filename, comment="#", dtype={'contig': str, 'position': int})
    dataset = dataset[dataset.judgement != "REJECT"]
    dataset = dataset[dataset.observed_in_normals_count == 0]
    return dataset[dataset['strand_bias_counts'].map(lambda x: strand_absolute_count(x))]
...
def main():
    parser = argparse.ArgumentParser(description="Filter Mutect 1.x variant calls based on callstats file")
    parser.add_argument("--callstat-file", help="Path to callstat file from Mutect 1.x", required=True)
    parser.add_argument("--vcf-file", help="Path to vcf file from Mutect 1.x", required=True)
    parser.add_argument("--out", help="Path to file to save filtered vcf in", required=True)
    ...
    for record in variant_file.fetch():
        if DEBUG:
            print("on record with location on chromosome {} at {}".format(record.chrom, record.pos))
        callstat_variant = filtered_callset[(filtered_callset.contig == record.chrom ) & \
